...
const FIFA_BY_DATE = new Map([
  ...
  ["2026-07-05T20:00", 91], ["2026-07-06T01:00", 92],
...
async function buildStats(events) {
  ...
  for (const data of summaries) {
    ...
    for (const kev of data.keyEvents ?? []) {
      ...
      // "goal*" = gol; "penalty---scored" = penal convertido en partido (tanda va en penaltyShootout, no en keyEvents)
      if (/^goal/.test(type) || type === "penalty---scored") {
...
function renderBracket(byNum) {
  ...
  // Bracket order (top→bottom). QF pairing: M89+M93→M97, M90+M94→M98, M91+M95→M99, M92+M96→M100
  // R32: (M73,M75)→M90, (M74,M77)→M89 → M97; (M83,M84)→M93, (M81,M82)→M94 → M98; (M76,M78)→M91, (M79,M80)→M92 → M99; (M85,M86)→M96, (M87,M88)→M95 → M100
  const ROUNDS = [
    { label: "1/16", nums: [73,75, 74,77, 83,84, 81,82, 76,78, 79,80, 85,87, 86,88] },
    { label: "1/8",  nums: [90,89, 93,94, 91,92, 96,95] },
